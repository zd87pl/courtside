...
## Overview
A 38-minute practice session: 10 strokes analyzed across 3 rallies, split 5 near / 5 far.
The dominant theme is late forehand preparation under time pressure, paired with a tactical
habit of not approaching on short balls. Depth control on the far side's backhand is the
...
1. **Short-ball approach ladder** - feed short balls; player must approach and close.
   Success: 8/10 approaches finished inside the service line. Targets short_ball_no_approach.
2. **Early-prep shadow + live** - racquet back before the bounce on a verbal cue.
   Success: contact in front of the front hip on 9/10 forehands. Targets late_preparation.
3. **Serve-target grid** - serve to called targets (wide / body / T), no repeats.
   Success: 12/15 to the called box. Targets serve_placement_predictable.

## Confidence Notes
Clip 3 is low confidence: the far player is partially out of frame at contact, so the
no_split_step read and the far stroke type are uncertain.
